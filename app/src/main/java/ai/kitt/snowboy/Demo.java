...
public class Demo extends Activity {
    ...
    private boolean checkPermission() {
        int result = checkSelfPermission("android.permission.ACCESS_WIFI_STATE");
        if (PackageManager.PERMISSION_DENIED == result) {
            return false;
        }
        result = checkSelfPermission("android.permission.RECORD_AUDIO");
        if (PackageManager.PERMISSION_DENIED == result) {
            return false;
        }
        result = checkSelfPermission("android.permission.WRITE_EXTERNAL_STORAGE");
        if (PackageManager.PERMISSION_DENIED == result) {
            return false;
        }
        result = checkSelfPermission("android.permission.READ_EXTERNAL_STORAGE");
        if (PackageManager.PERMISSION_DENIED == result) {
            return false;
        }
        return true;
    ...
    private void reqPermission() {
        requestPermissions(new String[]{
                ...
                "android.permission.WRITE_EXTERNAL_STORAGE", "android.permission.READ_EXTERNAL_STORAGE"}, 1);
    }
